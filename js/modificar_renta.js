...
let cacheEquiposEdicion = new Map();   // Caché de equipos ya buscados
let procesandoEscaneoEdicion = false;  // Flag para evitar escaneos simultáneos
let colaEscaneosEdicion = [];          // Cola de códigos pendientes
let contadorItemsEdicion = 0;          // Contador para IDs únicos
...
async function inicializarModificarRenta() {
  ...
  if (inputEquipo) {
    ...
    // ✅ Detectar Enter para agregar equipo (el lector USB envía Enter automáticamente)
...
function renderizarPaginacion(totalRegistros) {
  ...
  let html = '';
  html += `<button type="button" onclick="cambiarPagina(${paginaActualModificar - 1})"
...
async function cambiarPagina(nuevaPagina) {
  const totalPaginas = Math.ceil((rentasCache.length || 1) / POR_PAGINA_MODIFICAR);
...
function limpiarFiltros() {
  const elCliente = document.getElementById('filtroCliente');
  const elNumero = document.getElementById('filtroNumero');
...
async function seleccionarRentaModificar(numeroRenta) {
  try {
    ...
    // ✅ Asignar IDs únicos a los items existentes
    contadorItemsEdicion = 0;
    itemsEdicion = (items || []).map(item => ({
      ...item,
      _uid: 'item_' + (++contadorItemsEdicion) + '_' + Date.now()
    }));
...
if (btnReimprimir) btnReimprimir.style.display = 'inline-block';
    ...
    const inputEquipo = document.getElementById('editBuscarEquipo');
    if (inputEquipo) setTimeout(() => inputEquipo.focus(), 300);
...
// ==========================================
// RENDERIZAR ITEMS EN EDICIÓN (solo para carga inicial o eliminación)
// ==========================================
...
// ==========================================
// ✅ CREAR FILA DE ITEM (reutilizable)
// ==========================================
function crearFilaItem(item, index) {
  const fila = document.createElement('tr');
  fila.id = `fila-edicion-${item._uid}`;
  fila.innerHTML = `
    <td class="celda-numero">${index + 1}</td>
    ...
    <td style="text-align: center;">
      <input type="number" min="1" value="${item.cantidad}"
        ...
        onchange="actualizarCantidadEdicionPorUid('${item._uid}', this.value)">
    </td>
    <td style="text-align: right;" id="subtotal-edicion-${item._uid}"><strong>$${parseFloat(item.subtotal).toFixed(2)}</strong></td>
    <td style="text-align: center;">
      <button type="button" onclick="eliminarItemEdicionPorUid('${item._uid}')"
...
// ==========================================
function agregarFilaEdicion(item, index) {
  const tbody = document.getElementById('editTbodyItems');
  if (!tbody) return;
...
// ==========================================
// ✅ ACTUALIZAR CANTIDAD POR UID (SIN RE-RENDERIZAR)
// ==========================================
function actualizarCantidadEdicionPorUid(uid, cantidad) {
  const qty = parseInt(cantidad) || 1;
  const item = itemsEdicion.find(i => i._uid === uid);
  if (!item) return;

  item.cantidad = qty;
  item.subtotal = parseFloat(item.precio_unitario) * qty;

  // ✅ Actualizar solo la celda del subtotal de esta fila
  const celdaSubtotal = document.getElementById(`subtotal-edicion-${uid}`);
  if (celdaSubtotal) {
...
// ==========================================
// ✅ ELIMINAR ITEM POR UID
// ==========================================
function eliminarItemEdicionPorUid(uid) {
  if (!confirm('¿Eliminar este equipo de la renta?')) return;

  const index = itemsEdicion.findIndex(i => i._uid === uid);
  if (index === -1) return;

  itemsEdicion.splice(index, 1);

  // ✅ Eliminar la fila del DOM directamente
  const fila = document.getElementById(`fila-edicion-${uid}`);
  if (fila) fila.remove();
...
// ==========================================
function actualizarNumerosFilasEdicion() {
  itemsEdicion.forEach((item, index) => {
    const fila = document.getElementById(`fila-edicion-${item._uid}`);
    if (fila) {
      const celdaNumero = fila.querySelector('.celda-numero');
...
// ==========================================
// ✅ CALCULAR TOTALES (SIN RE-RENDERIZAR)
// ==========================================
...
// ==========================================
// ✅ BUSCAR EQUIPO EN BD (CON CACHÉ Y TIMEOUT)
// ==========================================
async function buscarEquipoEnBDEdicion(codigoBusqueda) {
  // ✅ 1. Verificar caché primero (búsqueda case-insensitive)
  const codigoUpper = codigoBusqueda.toUpperCase();
  if (cacheEquiposEdicion.has(codigoUpper)) {
    return cacheEquiposEdicion.get(codigoUpper);
  }
  ...
  try {
    const searchPromise = (async () => {
      ...
      // 1️⃣ Primero buscar por código de barras EXACTO (case-insensitive)
      ...
      // 2️⃣ Si no encontró, buscar por código de barras SIN guiones (por si el lector no los envía)
      const codigoSinGuiones = codigoBusqueda.replace(/-/g, '');
      if (codigoSinGuiones !== codigoBusqueda) {
        const resultado2 = await supabaseClient
          .from('equipos')
          .select('*')
          .select('*')
          .maybeSingle();
        
        // Buscar manualmente sin guiones en código de barras
        const { data: dataTodos } = await supabaseClient
          .from('equipos')
          .select('*')
          .limit(1000);
        
        if (dataTodos) {
          const encontrado = dataTodos.find(e => 
            e.codigo_barras && e.codigo_barras.replace(/-/g, '').toUpperCase() === codigoSinGuiones.toUpperCase()
          );
          if (encontrado) return encontrado;
        }
      }

      // 3️⃣ Si aún no encontró, buscar por serial (case-insensitive)
      const resultado3 = await supabaseClient
        .from('equipos')
        .select('*')
        .ilike('serial', codigoBusqueda)
        .maybeSingle();

      return resultado3.data;
    })();
    ...
    if (data) {
      cacheEquiposEdicion.set(codigoUpper, data);
    }
...
// ==========================================
// ✅ AGREGAR EQUIPO EN EDICIÓN (OPTIMIZADO CON COLA Y CACHÉ)
// ==========================================
async function agregarEquipoEdicion() {
  ...
  if (!codigo) {
    mostrarMensajeModificar('Por favor ingrese un código de barras o serial', 'error');
    input.focus();
    return;
  }

  // ✅ Sanitizar el código (NO deformarlo, solo limpiar caracteres peligrosos)
  codigo = codigo.replace(/'/g, '-').replace(/"/g, '-').replace(/`/g, '-').trim();
  input.value = ''; // ✅ Limpiar input inmediatamente para el siguiente escaneo
  input.focus();

  // ✅ Agregar a la cola y procesar
  colaEscaneosEdicion.push(codigo);
  procesarColaEscaneosEdicion();
}

// ==========================================
// ✅ PROCESAR COLA DE ESCANEOS (EVITA SATURACIÓN)
// ==========================================
async function procesarColaEscaneosEdicion() {
  if (procesandoEscaneoEdicion) return;
  procesandoEscaneoEdicion = true;

  while (colaEscaneosEdicion.length > 0) {
    const codigo = colaEscaneosEdicion.shift();
    try {
      await procesarEscaneoIndividualEdicion(codigo);
    } catch (err) {
      console.error('Error procesando escaneo:', err);
      mostrarMensajeModificar(`Error al procesar: ${codigo}`, 'error');
    }
  }

  procesandoEscaneoEdicion = false;
}

// ==========================================
// ✅ PROCESAR ESCANEO INDIVIDUAL
// ==========================================
async function procesarEscaneoIndividualEdicion(codigo) {
  // ✅ Buscar equipo (con caché)
  const equipo = await buscarEquipoEnBDEdicion(codigo);

  if (!equipo) {
    mostrarMensajeModificar(`Equipo no encontrado: "${codigo}"`, 'error');
    return;
  }

  // ✅ Verificar si ya está en la lista
  const existe = itemsEdicion.some(item => item.codigo_barras === equipo.codigo_barras);
  if (existe) {
    mostrarMensajeModificar('⚠️ Este equipo ya está en la renta', 'error');
    return;
  }

  const precioUnitario = equipo.costo || 0;
  const nuevoItem = {
    _uid: 'item_' + (++contadorItemsEdicion) + '_' + Date.now(), // ✅ ID único
  ...
  itemsEdicion.push(nuevoItem);

  // ✅ Agregar SOLO la nueva fila (sin re-renderizar todo)
  agregarFilaEdicion(nuevoItem, itemsEdicion.length - 1);

  // ✅ Actualizar solo los totales
  calcularTotalesEdicion();

  mostrarMensajeModificar(`✅ Equipo agregado: ${equipo.nombre_equipo} (${itemsEdicion.length} total)`, 'exito');
}

// ==========================================
// GUARDAR CAMBIOS (CON LIMPIEZA AUTOMÁTICA Y LOTES)
// ==========================================
async function guardarCambiosRenta() {
  ...
  try {
    ...
    // ✅ 3️⃣ Insertar items en LOTES DE 100 (mucho más rápido)
    const TAMANO_LOTE = 100;
    for (let i = 0; i < itemsEdicion.length; i += TAMANO_LOTE) {
      const lote = itemsEdicion.slice(i, i + TAMANO_LOTE).map(item => ({
