...
let statsHist = { activas: 0, vencidas: 0, terminadas: 0 };
...
async function cargarEstadisticasHist() {
  try {
    ...
    // Activas
    const { count: countActivas } = await supabaseClient
      .from('rentas')
      .select('*', { count: 'exact', head: true })
      .eq('estado', 'activa');

    // Vencidas
    const { count: countVencidas } = await supabaseClient
      .from('rentas')
      .select('*', { count: 'exact', head: true })
    ...
    // Terminadas
    const { count: countTerminadas } = await supabaseClient
      .from('rentas_terminadas')
      .select('*', { count: 'exact', head: true });

    statsHist = {
      activas: countActivas || 0,
      vencidas: countVencidas || 0,
      terminadas: countTerminadas || 0
    };

    document.getElementById('statActivas').textContent = statsHist.activas;
    document.getElementById('statVencidas').textContent = statsHist.vencidas;
    document.getElementById('statTerminadas').textContent = statsHist.terminadas;
...
async function marcarRecibidaHist(numeroRenta) {
  const confirmacion = confirm(`¿Confirmar que la renta #${numeroRenta} ha sido recibida?\n\nEsta acción moverá la renta a "Rentas Terminadas" y registrará la fecha real de devolución.`);
  
  if (!confirmacion) return;

  try {
    // Cargar datos completos de la renta
    ...
    // 1. Insertar en rentas_terminadas
    const { data: rentaTerminada, error: errorTerminada } = await supabaseClient
      ...
      .insert({
        ...
        observaciones_terminacion: `Recibida el ${hoy.toLocaleDateString('es-ES')}${diasAnticipados ? ` (${diasAnticipados} días antes)` : ''}${diasRetraso ? ` (${diasRetraso} días tarde)` : ''}`
      })
      .select()
      .single();
    ...
    // 2. Insertar items en rentas_items_terminadas
    ...
    // 3. Eliminar items de rentas_items
    ...
    if (typeof registrarLog === 'function') {
      const descripcion = `Renta #${numeroRenta} recibida y terminada | Cliente: ${renta.cliente_nombre} | Total: $${parseFloat(renta.total).toFixed(2)} | ${diasAnticipados ? `${diasAnticipados} días antes` : ''}${diasRetraso ? `${diasRetraso} días tarde` : 'A tiempo'} | Recibido por: ${usuarioActualHist?.email || 'Desconocido'}`;
