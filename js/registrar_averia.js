...
// ==========================================
// ✅ BUSCAR EQUIPO (VINCULADO AL WINDOW PARA EL ONCLICK)
// ==========================================
...
async function cargarFotosDelEquipo(equipo) {
  // Esta función es opcional si ya tienes las fotos en la ficha, 
  // pero la dejamos por si quieres mostrarlas en el registro de avería.
}
...
function renderizarPreviewFotosEvidencia() {
  ...
  fotosEvidencia.forEach((fotoUrl, index) => {
    ...
    div.innerHTML = `
      ...
      <button type="button" class="foto-remove" onclick="event.stopPropagation(); window.eliminarFotoEvidencia(${index})">✕</button>
    `;
...
window.guardarAveria = async function() {
  ...
  try {
    const insertData = {
      ...
      // ✅ NUEVO: Guardar fotos originales del equipo para poder restaurarlas al reintegrar
    ...
    setTimeout(() => {
      window.limpiarFormularioAveria(true);
    }, 1500);
